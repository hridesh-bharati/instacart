...
export default function HomeScreen({ onAddToCart }) {
  ...
  useEffect(() => {
    // 1. Flash Sale Live Listener
    const unsubFlash = onSnapshot(collection(db, 'flashSale'), (snapshot) => {
      setFlashSaleList(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
    });

    // 2. Featured Stores Live Listener
    const unsubStores = onSnapshot(collection(db, 'featuredStores'), (snapshot) => {
      setStoresList(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
    });

    // 3. Daily Essentials Live Listener
    const unsubEssentials = onSnapshot(collection(db, 'dailyEssentials'), (snapshot) => {
      setEssentialsList(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
      setLoading(false);
    });
...
const styles = StyleSheet.create({
  ...
  categoryRow: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, marginVertical: 12 },
  categoryItem: { alignItems: 'center', gap: 6 },
  categoryCircle: { width: 58, height: 58, borderRadius: 29, justifyContent: 'center', alignItems: 'center' },
  categoryTitle: { fontSize: 12, color: colors.textDark, fontWeight: '500' },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, marginTop: 18, marginBottom: 12 },
